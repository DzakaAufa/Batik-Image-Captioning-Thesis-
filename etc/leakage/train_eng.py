...
def train_qwen_model(
    ...
    train_df = full_df[full_df["new_split"] == "train"].reset_index(drop=True)
    val_df   = full_df[full_df["new_split"] == "val"].reset_index(drop=True)
    test_df  = full_df[full_df["new_split"] == "test"].reset_index(drop=True)
    unused_df = full_df[full_df["new_split"] == "unused"]
...
if __name__ == "__main__":
    model, processor = train_qwen_model(
        ...
        caption_dir="/mnt/extended-home/dzakaaufa/leakage/model150_65/data150.csv",
        output_dir="/mnt/extended-home/dzakaaufa/leakage/model150_65/qwen",
        epochs=10,
        batch_size=8,
        lr=1e-4, 
        model_size="Qwen/Qwen2.5-VL-7B-Instruct",
