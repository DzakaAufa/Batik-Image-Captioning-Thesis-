...
def generate_caption(
    ...
    """
    Jalankan zero-shot inferensi untuk satu gambar.

    Returns:
        String caption yang dihasilkan model.
    """
    ...
    with torch.autocast(device_type="cuda" if "cuda" in device else "cpu", dtype=dtype):
        output_ids = model.generate(
            **inputs,
            ...
            do_sample=False,          # greedy — deterministik untuk zero-shot
            temperature=1.0,
            repetition_penalty=1.1,   # kurangi pengulangan frasa
        )

    # Potong token prompt — ambil hanya bagian yang digenerate
...
def run_zero_shot(
    ...
    """
    Iterasi seluruh baris CSV, generate caption zero-shot,
    dan simpan hasilnya ke CSV baru.

    Args:
        csv_path      : CSV input dengan kolom 'image_path' dan 'class'
        image_dir     : Direktori root gambar (jika path di CSV relatif)
        output_csv    : Path output CSV hasil caption
        model_name    : Nama / path model Qwen
        use_4bit      : Gunakan kuantisasi 4-bit (hemat VRAM)
        max_new_tokens: Panjang maksimum caption
        resume        : Lanjutkan dari baris terakhir jika output sudah ada
    """
    ...
    # Resume support — skip baris yang sudah ada captionnya
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Zero-Shot Captioning"):
        ...
        # Simpan setiap 10 baris (checkpoint ringan)
        if len(results) % 10 == 0:
...
def parse_args():
    ...
    parser.add_argument(
        "--output",
        default="/mnt/extended-home/dzakaaufa/dataset/caption/data_B_zero_noclass1.csv",
