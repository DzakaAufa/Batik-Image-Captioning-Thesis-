...
OUTPUT_DIR = "/mnt/extended-home/dzakaaufa/models/resnet"

IMG_SIZE   = 224
BATCH_SIZE = 16  
ACCUM_ITER = 2   # Efektif batch = 32
EPOCHS     = 10
...
# ============================================================
# TRANSFORM (DETOKS AUGMENTASI - RAMAH BATIK)
# ============================================================
...
def train_model(model, train_loader, val_loader, num_classes, class_weights):
    ...
    # PERBAIKAN: Membuat variabel penampung prediksi terbaik
    best_gts = []
    best_preds = []
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch == 5:
            print("🔥 Partial Unfreezing (Layer 4 & FC Layer)")
        ...
        # PERBAIKAN: Simpan data evaluasi hanya jika mendapatkan akurasi terbaik
        if acc > best_acc:
            best_acc = acc
            best_gts = gts.copy()
            best_preds = preds.copy()
            torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, "best_resnet_clean.pth"))
...
def main():
    ...
    model = build_resnet_model(len(classes))
    ...
    # ============================================================
    # PERBAIKAN: PLOT CONFUSION MATRIX
    # ============================================================
    ...
    plt.title('Confusion Matrix - ResNet50', fontsize=15, fontweight='bold', pad=20)
    plt.xticks(rotation=45, ha='right')
    plt.yticks(rotation=0)
    plt.tight_layout()

    # Simpan hasil confusion matrix ke direktori output
